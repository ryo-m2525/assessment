...
assessmentButton.addEventListener( // イベント検知設定の追加
  ...
  () => { // 無名関数でアロー関数、イベント検知をしたら実行される
    ...
    //console.log(userName); // ログ出力をして確認
    ...
    //tweetDivision.innerText = ''; // Tweetのdivタグを空文字で上書きする
    ...
    const script = document.createElement('script'); // Scriptタグを追加
...
function assessment(userName) {
  ...
  */
  ...
  const index = sumOfcharCode % answers.length; // 合計値を配列の要素数で余るをとることで、配列の要素数の数値に収める
//  const result = answers[index]; // 配列から答えを取得
